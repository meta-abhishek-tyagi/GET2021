...
SELECT user.ID AS 'UserID', FirstName, LastName,
...
SELECT user.ID AS 'UserID', FirstName, LastName, SUM(ProductQuantity * ProductPrice) AS 'Revenue'
FROM user
JOIN orders ON user.ID = orders.UserID
JOIN order_detail ON order_detail.OrderID = orders.ID
WHERE DATEDIFF(NOW(), orders.OrderTime) <= 30 AND order_detail.Status = 'SHIPPED'
GROUP BY user.ID
ORDER BY Revenue DESC
LIMIT 10;

# Display top 20 Products which are ordered most in last 60 days along with numbers.
SELECT product.ID AS 'ProductID', Title, SUM(ProductQuantity) AS 'Number of Orders'
FROM orders
JOIN order_detail ON order_detail.OrderID = orders.ID
...
SELECT MONTHNAME(orders.OrderTime) as 'Month', SUM(ProductQuantity * ProductPrice) AS 'Monthly Sales Revenue'
FROM orders
JOIN order_detail ON order_detail.OrderID = orders.ID
WHERE orders.OrderTime >= CURDATE() - INTERVAL 6 MONTH AND order_detail.Status = 'SHIPPED'
...
WHERE product.ID NOT IN 
    ( SELECT product.ID 
      FROM ( SELECT product.ID
             ...
             WHERE DATEDIFF(NOW(), OrderTime) <= 90 )
             AS C
    );
...
SELECT product.ID AS 'ProductID', product.Title
FROM product
WHERE product.CategoryID IN (SELECT  category.ID
                             FROM category
                             WHERE category.parentID = (SELECT category.ID
                                                        FROM category
                                                        WHERE category.Title = 'Mobile Phones'));

# Display top 10 Items which were cancelled most.
SELECT product.ID, Title, SUM(ProductQuantity) AS 'Cancellation Count'
FROM order_detail
JOIN product ON order_detail.ProductID = product.ID
