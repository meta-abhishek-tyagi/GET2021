...
SELECT orders.ID AS 'OrderID', DATE(OrderTime) AS 'Order Date', SUM(ProductPrice*ProductQuantity) AS Total
FROM orders
...
SELECT orders.ID AS 'OrderID', SUM(ProductPrice*ProductQuantity) AS Total, UserID
FROM orders
JOIN order_detail ON orders.ID = order_detail.OrderID
GROUP BY orders.ID
ORDER BY Total DESC
...
SELECT orders.ID AS 'OrderID', DATE(OrderTime) AS 'Order Date', UserID
FROM orders
WHERE DATEDIFF(NOW(), OrderTime) > 10 AND ID IN (SELECT DISTINCT OrderID
FROM order_detail
WHERE status = 'PLACED');
...
SELECT UserID, FirstName, LastName, DATEDIFF(NOW(), OrderTime) AS 'Days Since Inactive'
FROM orders
JOIN user ON orders.UserID = user.ID
WHERE DATEDIFF(NOW(), OrderTime) >= 30 AND user.Role = 'SHOPPER';

# Display list of shopper along with orders placed by them in last 15 days.
SELECT UserID, FirstName, LastName, orders.ID AS 'OrderID', ProductID, Title, ProductPrice, DATE(OrderTime) AS 'Order Date'
FROM user
...
SELECT OrderID, ProductID, Title, order_detail.Status
FROM order_detail
JOIN product ON order_detail.ProductID = product.ID
WHERE order_detail.Status = 'SHIPPED' AND OrderID = '1';

# Display list of order items along with order placed date 
# which fall between Rs. 1000 to Rs. 10000 price.
SELECT ProductID, Title, ProductPrice, DATE(OrderTime) AS 'Order Date'
FROM product
JOIN order_detail ON order_detail.ProductID = product.ID
